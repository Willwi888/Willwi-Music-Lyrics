...
const App: React.FC = () => {
  ...
  const handleAiGenerate = () => {
    ...
    const password = prompt('請輸入密碼以啟用 AI 功能：\n\n提醒：AI 功能會使用 Google Gemini API 並可能產生費用，請注意您的用量。');
    if (password === '8888') {
      runAiGeneration();
  ...
  const renderContent = () => {
    switch (appState) {
      case 'WELCOME':
        return (
            <div 
              className="text-center text-white flex flex-col items-center justify-center gap-6 cursor-pointer group" 
              onClick={() => setAppState('FORM')}
            >
              <DogLogo className="w-56 h-56 rounded-full object-cover shadow-lg transition-transform duration-300 group-hover:scale-105" />
              <h2 className="text-2xl font-semibold tracking-widest text-gray-400 transition-colors group-hover:text-white" style={{ fontFamily: 'serif' }}>
                天選之桶
              </h2>
              <p className="text-sm text-gray-500">點擊任意處進入</p>
            </div>
        );
      case 'TIMING':
        return (
          ...
          <div className="w-full max-w-lg p-8 space-y-6 bg-gray-800/50 backdrop-blur-sm rounded-xl shadow-2xl border border-gray-700">
            ...
            <form onSubmit={handleStart} className="space-y-6">
                    ...
                    <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                        {backgroundUrls.map((url, index) => (
                            <div key={index} className="relative group">
                                ...
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 4.293a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                </button>
                            </div>
                        ))}
                    </div>
              ...
              <div>
                <button
                  type="submit"
                  className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-bold text-gray-900 bg-[#a6a6a6] hover:bg-[#999999] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-gray-500 transition-all duration-300"
                >
                  開始對時
                </button>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 flex items-center justify-center p-4 font-sans transition-opacity duration-500" style={{ opacity: isMounted ? 1 : 0 }}>
      {renderContent()}
    </div>
  );
};
